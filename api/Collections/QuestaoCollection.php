...
class QuestaoCollection implements ICollection {
	...
	public function get(array $params = array()) {
        ...
        } else if(isset($params['tipo'], $params['habilidade'])) {
            return $this->getByTipoAndHabilidade($params['tipo'], $params['habilidade'], $status);
        }
    ...
    public function getAllFiltered($params) {
        ...
        if(!empty($params['questoes'])) {
            array_push($where, "q.id IN (:questoes)");
        } else {
        ...
        $sql .= " WHERE " . implode(' AND ', $where) . " ORDER BY RAND() LIMIT :numero_questoes";
        
        $params = array('status' =>  1, 'numero_questoes' => $params['numero_questoes']);
    ...
    private function getByTipoAndHabilidade($tipo, $habilidade, $status) {
        $sql = "SELECT * FROM " . self::_TABLE . " WHERE FIND_IN_SET(tipo, :tipo) AND FIND_IN_SET(habilidade_Id, :habilidade_id)";
        $params = array("tipo" => $tipo, 'habilidade_id' => $habilidade);
    ...
    private function validateItens($tipo, $itens, &$errors) {
        ...
        if(is_null($tipo)) {
            ...
            if(count($itens) == 0) {
                array_push($errors, 'Questões do tipo ' . Util::filter($tipo, 'questionType') . ' devem possuir pelo menos duas alternativas');
            } else if(count($itens) > 5) {
                array_push($errors, 'questões do tipo ' . Util::filter($tipo, 'questionType') . ' devem possuir no máximo cinco alternativas');
            }   

           


            $itensCorretos = array_map(create_function('$item', 'return new Api\Models\Item($item);'), array_filter($itens, create_function('$item', 'return $item->getCorreto() == true || $item->getCorreto() == 1;')));


            if($tipo == 'MULTIPLA_ESCOLHA') {
                if(count($itensCorretos) > 1) {
                    array_push($errors, 'Questões do tipo ' . Util::filter($tipo, 'questionType') . ' devem possuir uma única alternativa correta');
                } else if(count($itensCorretos) == 0) {
                    array_push($errors, 'Questões do tipo ' . Util::filter($tipo, 'questionType') . ' devem possuir pelo menos uma alternativa correta');
                }
    ...
    public function validateBatch($tipo, $itens) {
        $errors = array();
